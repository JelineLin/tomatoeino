...
struct APIClient {
    // 后端地址：HTTPS + 域名 + DigiCert 正证书（系统信任链，无需任何 pinning/描述文件）。
    // 2026-07-16 jelinelin.com 阿里云接入备案批复，SNI 拦截解除，从明文 HTTP 回到 TLS：
    // 服务端 443 + certs/jelinelin.com.pem，token 从此不再明文过网。
    var baseURL = URL(string: "https://jelinelin.com")!
    // var baseURL = URL(string: "http://127.0.0.1:8080")!  // ← 本地调试用（需临时加回 ATS 豁免）
    ...
    // submitFeedback 给某道菜记「儿童食用反馈」（POST /api/history/feedback）。
    // dish 传菜名（菜级粒度）；rating 传 like/dislike/ok，空串表示清除该菜反馈。
    // 后端写完返回整份历史，直接拿去刷新。
    func submitFeedback(date: String, meal: String, dish: String, rating: String, note: String) async throws -> [Day] {
        ...
        req.httpBody = try JSONEncoder().encode(Body(date: date, meal: meal, dish: dish, rating: rating, note: note))
