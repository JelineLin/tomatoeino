...
A Makefile covers the routine flows: `make build` / `make test` / `make linux` (cross-compile the server for hermas into `dist/`, gitignored), `make release VERSION=vX.Y.Z` (tag + upload the linux binary to GitHub Releases via `gh` — binaries are never committed to the repo), `make deploy` (rsync with resume/retries + md5 check to hermas temp path, then `sudo mv` — direct overwrite of a running binary hits "text file busy", and plain scp of the 27M binary tends to drop mid-transfer — then restart + healthz check), `make web` (Next.js static export → `web/out`), `make deploy-web` (rsync the static site to hermas — no server restart needed, spaHandler reads disk per request).

**`web/`** is the Next.js (App Router, TS + Tailwind) web client. It builds to a pure static export (`next build` → `web/out`, config switches on build phase) served by the Go server's `spaHandler` on the same origin — so the browser calls `/api/*` directly with no CORS. In dev, `npm run dev` proxies `/api/*` to a local Go server via rewrites (`API_PROXY` env to point elsewhere). Token lives in localStorage behind a token-gate shell; any 401 broadcasts an event that drops back to the gate. The SSE chat protocol and per-dish feedback semantics mirror the iOS client exactly (`web/lib/api.ts` is the single API chokepoint, like iOS `APIClient`).
...
- **`examples/NN_<topic>/main.go`** — each is its own `package main`. New examples follow the same numbering convention and reuse `internal/llm`.
  ...
  - `02_menu_agent` — the menu-planning ReAct agent as a CLI: builds the same shared business core (`menu.BuildAgent` over `internal/menu`) that `cmd/server` serves over HTTP, then streams one answer. Its `data/history.json` is the seed toddler meal history (lunch/fruit/dinner per day).
  ...
  - `04_graph_react` — hand-builds a mini-ReAct with `compose.Graph`, opening the `react.NewAgent` black box: a `prompt.ChatTemplate` node, model/tools nodes sharing per-run state (`WithGenLocalState` + node pre/post handlers accumulate the message history), a `NewGraphBranch` routing on `tool_calls`, and the tools→model back-edge that *is* the ReAct loop (cyclic graphs require `WithNodeTriggerMode(AnyPredecessor)`; `WithMaxRunSteps` is the loop fuse). Part B recompiles the same graph with `WithCheckPointStore` + `WithInterruptBeforeNodes` for native interrupt-resume (approve-before-write): the first `Invoke` returns an error unpacked via `ExtractInterruptInfo`, a second `Invoke` with the same `WithCheckPointID` resumes from the checkpoint without replaying the first half. This is exactly the compile-option slot `react.NewAgent` (v0.9.5) doesn't expose — the reason L3 used `ToolReturnDirectly` instead. Custom state types must be registered with `schema.RegisterName` for checkpoint serialization.
