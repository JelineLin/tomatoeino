...
- 建档职责：家长提到宝宝的生日或月龄、过敏、忌口、饮食要点时，你必须调用 update_profile
  记入档案——家长说月龄的，按今天的日期换算成出生日期存；过敏原/忌口是【整组覆盖】，
  修改时把要保留的项一起带全。档案会自动出现在你的上下文里（【宝宝档案】一段），
  不需要也没有查询工具。若上下文里没有【宝宝档案】且历史为空（新家庭首次使用），
  先用 ask_user 了解宝宝月龄和过敏/忌口并建档，再开始推荐。
...
//
// Assembly 是 BuildAgent 的全部产出：agent 本体 + 各个可写 store。
// store 同时交给 HTTP 层做只读出口（/api/history、/api/inventory、/api/profile）——
// 和 agent 的工具共用同一本账，一份数据两个出口，说法永不打架。
...
// 轻资产（账本、向量索引、agent 图、档案）。注入也让测试可以塞假实现，全程离线。
func BuildAgent(ctx context.Context, embedder embedding.Embedder, cm model.ToolCallingChatModel,
	historyPath, inventoryPath, profilePath string) (*Assembly, error) {
	// 1. 打开历史账本 + 库存账本 + 档案（都是带锁可写的 store，工具会运行时写入）
	...
	ps, err := NewProfileStore(profilePath)
	if err != nil {
		return nil, err
	}
	...
	if err := store.Add(ctx, BuildDocuments(hs.Snapshot())); err != nil {
		return nil, fmt.Errorf("把历史灌进向量库失败: %w", err)
	}

	// 3. 工具集
	tools, err := NewTools(store, hs, inv, ps)
	if err != nil {
		return nil, err
	}
	...
	agent, err := react.NewAgent(ctx, &react.AgentConfig{
		...
		// 人设 + 当天日期 + 宝宝档案，每个请求动态注入（而非建图时写死）。
		// 日期必须给：模型自己不知道今天几号，不给就瞎猜（实测把月份猜成 4）。
		// 档案动态取值的意义：update_profile 刚记完，下一轮人设里就是新档案——
		// 建档立即生效，agent 不用重建；月龄也按当天现算，跨月不过期。
