...
func (s *Store) EnsureProfile(ctx context.Context, userID string) (Profile, error) {
	...
	now := time.Now().UTC()
	_, err := s.db.ExecContext(ctx, `INSERT INTO learning_profiles
 (user_id,cet4_score,level,daily_minutes,goal,difficulty,navigation_position,content_mode,ielts_track,updated_at)
 VALUES(?,?,?,?,?,?,?,?,?,?) ON CONFLICT(user_id) DO NOTHING`,
		userID, 430, DefaultLevel, DefaultDailyMinutes, "提升英文阅读与口语", 1, DefaultNavigation, DefaultContentMode, DefaultIELTSTrack, now.Format(time.RFC3339Nano))
	if err != nil {
		return Profile{}, err
	}
...
func (s *Store) Profile(ctx context.Context, userID string) (Profile, error) {
	var p Profile
	var updated string
	err := s.db.QueryRowContext(ctx, `SELECT user_id,cet4_score,level,daily_minutes,goal,difficulty,navigation_position,content_mode,ielts_track,updated_at
...
func (s *Store) SaveProfile(ctx context.Context, p Profile) (Profile, error) {
 ...
 ielts_track=excluded.ielts_track,updated_at=excluded.updated_at`,
		p.UserID, p.CET4Score, p.Level, p.DailyMinutes, p.Goal, p.Difficulty, p.Navigation, p.ContentMode, p.IELTSTrack, p.UpdatedAt.Format(time.RFC3339Nano))
	if err != nil {
		return Profile{}, err
	}
	return p, nil
...
func (s *Store) PutLesson(ctx context.Context, l Lesson) (Lesson, bool, error) {
	...
	res, err := s.db.ExecContext(ctx, `INSERT INTO lessons
 ...
 VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(user_id,lesson_date) DO NOTHING`,
		l.UserID, l.Date, l.Title, l.Passage, marshal(l.Vocabulary), marshal(l.Questions), l.Difficulty, l.GenerationReason, l.EstimatedMinutes,
		l.ContentMode, l.ExerciseStyle, l.SyllabusFocus, l.SourceName, l.SourceTitle, l.SourceURL, l.SourcePublished, l.AdaptationNote, l.ContentHash, l.CreatedAt.Format(time.RFC3339Nano))
	if err != nil {
...
func scanLesson(row interface{ Scan(...any) error }) (Lesson, error) {
	var l Lesson
	var vocab, questions, created string
	err := row.Scan(&l.ID, &l.UserID, &l.Date, &l.Title, &l.Passage, &vocab, &questions, &l.Difficulty, &l.GenerationReason, &l.EstimatedMinutes,
...
func (s *Store) LessonByDate(ctx context.Context, userID, date string) (Lesson, error) {
	return scanLesson(s.db.QueryRowContext(ctx, `SELECT `+lessonColumns+` FROM lessons WHERE user_id=? AND lesson_date=?`, userID, date))
}

func (s *Store) Lesson(ctx context.Context, userID string, id int64) (Lesson, error) {
	return scanLesson(s.db.QueryRowContext(ctx, `SELECT `+lessonColumns+` FROM lessons WHERE user_id=? AND id=?`, userID, id))
}
...
func (s *Store) Lessons(ctx context.Context, userID string, limit int) ([]Lesson, error) {
	...
	rows, err := s.db.QueryContext(ctx, `SELECT `+lessonColumns+` FROM lessons WHERE user_id=? ORDER BY lesson_date DESC LIMIT ?`, userID, limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Store) RecentSourceURLs(ctx context.Context, userID string, limit int) ([]string, error) {
	...
	rows, err := s.db.QueryContext(ctx, `SELECT source_url FROM lessons WHERE user_id=? AND source_url<>'' ORDER BY lesson_date DESC LIMIT ?`, userID, limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Store) SaveReadingAttempt(ctx context.Context, userID string, lessonID int64, answers []Answer) (ReadingAttempt, error) {
	...
	now := time.Now().UTC()
	res, err := s.db.ExecContext(ctx, `INSERT INTO reading_attempts
 (user_id,lesson_id,answers_json,correct_count,total_count,accuracy,completed_at) VALUES(?,?,?,?,?,?,?)`,
		userID, lessonID, marshal(answers), correct, total, accuracy, now.Format(time.RFC3339Nano))
	if err != nil {
		return ReadingAttempt{}, err
	}
...
func (s *Store) LatestReadingAttempt(ctx context.Context, userID string, lessonID int64) (ReadingAttempt, error) {
	...
	var answers, completed string
	err := s.db.QueryRowContext(ctx, `SELECT id,user_id,lesson_id,answers_json,correct_count,total_count,accuracy,completed_at
...
func (s *Store) SaveSpeakingAttempt(ctx context.Context, a SpeakingAttempt) (SpeakingAttempt, error) {
	...
	defer tx.Rollback()
	res, err := tx.ExecContext(ctx, `INSERT INTO speaking_attempts
 (user_id,lesson_id,audio_path,duration_seconds,transcript,wpm,accuracy,score,feedback,issues_json,completed_at)
 VALUES(?,?,?,?,?,?,?,?,?,?,?)`, a.UserID, a.LessonID, a.AudioPath, a.DurationSec, a.Transcript, a.WPM, a.Accuracy, a.Score, a.Feedback, marshal(a.Issues), now.Format(time.RFC3339Nano))
	if err != nil {
		return SpeakingAttempt{}, err
	}
	a.ID, _ = res.LastInsertId()
	for _, issue := range a.Issues {
		if _, err := tx.ExecContext(ctx, `INSERT INTO word_results(user_id,speaking_attempt_id,expected_word,actual_word,issue_kind,created_at) VALUES(?,?,?,?,?,?)`,
			a.UserID, a.ID, issue.Expected, issue.Actual, issue.Kind, now.Format(time.RFC3339Nano)); err != nil {
			return SpeakingAttempt{}, err
		}
		if issue.Expected != "" {
			if _, err := tx.ExecContext(ctx, `INSERT INTO weak_points(user_id,point_type,point_value,occurrence_count,last_seen_at) VALUES(?,?,?,?,?)
			 ON CONFLICT(user_id,point_type,point_value) DO UPDATE SET occurrence_count=occurrence_count+1,last_seen_at=excluded.last_seen_at`,
				a.UserID, "word", strings.ToLower(issue.Expected), 1, now.Format(time.RFC3339Nano)); err != nil {
				return SpeakingAttempt{}, err
			}
		}
	}
...
func (s *Store) SpeakingAttempt(ctx context.Context, userID string, id int64) (SpeakingAttempt, error) {
	...
	var issues, completed string
	err := s.db.QueryRowContext(ctx, `SELECT id,user_id,lesson_id,audio_path,duration_seconds,transcript,wpm,accuracy,score,feedback,issues_json,completed_at
...
func (s *Store) Progress(ctx context.Context, userID string, now time.Time) (ProgressSnapshot, error) {
	...
	if err := s.db.QueryRowContext(ctx, `SELECT COUNT(*) FROM lessons WHERE user_id=? AND lesson_date>=?`, userID, since).Scan(&assigned); err != nil {
		return ProgressSnapshot{}, err
	}
	if err := s.db.QueryRowContext(ctx, `SELECT COUNT(DISTINCT lesson_id) FROM (
	 SELECT lesson_id FROM reading_attempts WHERE user_id=? AND completed_at>=?
	 UNION SELECT lesson_id FROM speaking_attempts WHERE user_id=? AND completed_at>=?)`, userID, since, userID, since).Scan(&completed); err != nil {
		return ProgressSnapshot{}, err
	}
	var readAcc, speakAcc, wpm sql.NullFloat64
	_ = s.db.QueryRowContext(ctx, `SELECT AVG(accuracy) FROM reading_attempts WHERE user_id=? AND completed_at>=?`, userID, since).Scan(&readAcc)
	_ = s.db.QueryRowContext(ctx, `SELECT AVG(accuracy),AVG(wpm) FROM speaking_attempts WHERE user_id=? AND completed_at>=?`, userID, since).Scan(&speakAcc, &wpm)
	rows, err := s.db.QueryContext(ctx, `SELECT point_value FROM weak_points WHERE user_id=? AND point_type='word' ORDER BY occurrence_count DESC,last_seen_at DESC LIMIT 10`, userID)
	if err != nil {
		return ProgressSnapshot{}, err
	}
	defer rows.Close()
...
func (s *Store) WeeklyReports(ctx context.Context, userID string, limit int) ([]WeeklyReport, error) {
	...
	rows, err := s.db.QueryContext(ctx, `SELECT id,user_id,week_start,completion_rate,reading_accuracy,speaking_wpm,problem_words_json,summary,next_focus,created_at
 FROM weekly_reports WHERE user_id=? ORDER BY week_start DESC LIMIT ?`, userID, limit)
	...
	for rows.Next() {
		var r WeeklyReport
		var words, created string
		if err := rows.Scan(&r.ID, &r.UserID, &r.WeekStart, &r.CompletionRate, &r.ReadingAccuracy, &r.SpeakingWPM, &words, &r.Summary, &r.NextFocus, &created); err != nil {
			return nil, err
		}
...
func (s *Store) StartJob(ctx context.Context, userID, date, kind string) (bool, error) {
	...
	res, err := s.db.ExecContext(ctx, `INSERT INTO job_runs(user_id,job_date,job_type,status,notification_status,created_at,updated_at)
 VALUES(?,?,?,?,?,?,?) ON CONFLICT(user_id,job_date,job_type) DO UPDATE SET status='running',error_message='',updated_at=excluded.updated_at
 WHERE job_runs.status!='succeeded'`, userID, date, kind, "running", "pending", now, now)
	if err != nil {
		return false, err
	}
	n, _ := res.RowsAffected()
...
func (s *Store) FinishJob(ctx context.Context, userID, date, kind, status, notifyStatus, message string) error {
	_, err := s.db.ExecContext(ctx, `UPDATE job_runs SET status=?,notification_status=CASE WHEN ?='' THEN notification_status ELSE ? END,error_message=?,updated_at=?
	 WHERE user_id=? AND job_date=? AND job_type=?`, status, notifyStatus, notifyStatus, message, time.Now().UTC().Format(time.RFC3339Nano), userID, date, kind)
	return err
}
...
func (s *Store) ClaimNotification(ctx context.Context, userID, date, kind string) (bool, error) {
	res, err := s.db.ExecContext(ctx, `UPDATE job_runs SET notification_status='sending',updated_at=?
 WHERE user_id=? AND job_date=? AND job_type=? AND notification_status='pending'`, time.Now().UTC().Format(time.RFC3339Nano), userID, date, kind)
	if err != nil {
		return false, err
	}
	n, _ := res.RowsAffected()
...
func (s *Store) SaveWeeklyReport(ctx context.Context, r WeeklyReport) (WeeklyReport, error) {
	...
	_, err := s.db.ExecContext(ctx, `INSERT INTO weekly_reports(user_id,week_start,completion_rate,reading_accuracy,speaking_wpm,problem_words_json,summary,next_focus,created_at)
 ...
 summary=excluded.summary,next_focus=excluded.next_focus,created_at=excluded.created_at`, r.UserID, r.WeekStart, r.CompletionRate, r.ReadingAccuracy, r.SpeakingWPM, marshal(r.ProblemWords), r.Summary, r.NextFocus, r.CreatedAt.Format(time.RFC3339Nano))
	if err != nil {
...
func (s *Store) ApplyWeeklyDifficulty(ctx context.Context, userID, weekStart string, recommended int, reason string) error {
	...
	var count int
	if err := s.db.QueryRowContext(ctx, `SELECT COUNT(*) FROM plan_versions WHERE user_id=? AND created_at>=?`, userID, weekStart).Scan(&count); err != nil {
		return err
	}
	if count > 0 {
		return nil
	...
	defer tx.Rollback()
	now := time.Now().UTC().Format(time.RFC3339Nano)
	if _, err := tx.ExecContext(ctx, `UPDATE learning_profiles SET difficulty=?,updated_at=? WHERE user_id=?`, recommended, now, userID); err != nil {
		return err
	}
	if _, err := tx.ExecContext(ctx, `INSERT INTO plan_versions(user_id,old_difficulty,new_difficulty,reason,created_at) VALUES(?,?,?,?,?)`, userID, p.Difficulty, recommended, reason, now); err != nil {
		return err
	}
	return tx.Commit()
}
